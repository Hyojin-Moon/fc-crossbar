...
`react-native-svg` 로 직접 그린다 (Expo Go 에 포함된 모듈). 차트 라이브러리는 쓰지 않는다.

- **회원별 참석률 막대는 전부 같은 색**이다 (`Colors.accent`). 값에 따라 색을 진하게 하는
  value-ramp 는 안 쓴다 — 길이가 이미 크기를 말하고 있고, 색 채널을 낭비하는 안티패턴이다.
  본인 행은 색조가 아니라 배경 강조 + `나` 배지로 구분한다.
- **추세 차트는 단일 시리즈라 범례가 없다** (제목이 시리즈를 설명).
  모든 점에 숫자를 찍지 않고 마지막 점과 최고점만 직접 라벨을 붙인다.
- 눈금선은 실선 hairline 하나. 점선은 "예측/임계값"으로 읽히므로 쓰지 않는다.
- 차트 컨테이너 높이는 x축 라벨 자리(`AXIS_BAND`)를 포함해야 한다. 안 그러면 라벨이 잘린다.
- **필터는 화면 상단 한 줄에만 둔다.** 카드마다 필터를 넣지 않고 모든 카드가 같은 구간을 본다.

투표 색(참석 녹색 / 불참 빨강 / 미정 주황)은 색약 검증을 통과했다 (protan ΔE 10.7,
normal 20.6). 미정을 더 어둡게 하면 대비는 좋아지지만 불참과 구분이 안 된다(ΔE 5.5) —
현재 값을 유지하고, 색 옆에는 항상 텍스트 라벨을 둔다.
